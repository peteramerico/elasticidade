...
\node[below right] at (0,0) {$O = O'$};
  \draw[bend right=30,thick, ->] (e1) node[below]{$\pmb\phi$} to (e1rot);
  \draw[bend right=30,thick, ->] (e2)  to node[right]{$\pmb\beta$} (e2rot);
  \draw[bend right=30,thick, ->] (e3)  to node[above]{$\pmb\gamma$} (e3rot);
\end{tikzpicture}
\end{figure}
...
\begin{equation}
  ...
  \lvert e'_{j}\rvert\lvert e'_{k}\rvert \cos{\theta} \, .
\end{equation}
 Note que os elementos de
$\alpha_{ij}$ s\~ao cossenos, denominados cossenos direcionais, onde $\theta$
representa o \^angulo entre os vetores $\vec{e_{i}}'$ e $\vec{e_{k}}'$.
